...
public class WorkThread extends Thread {
    ...
    private void findNameByPhone() {
        String numberToFind = "+380111111111";
        // while(true) {
        data.lockRead();
        System.out.println("Searching name by phone.");
        for (Account account : data.accounts) {
            if (account.getPhoneNumber().equals(numberToFind))
    ...
    private void RWAccounts() {
        ...
        if (tt)
            data.accounts.add(a1);
        else
            data.accounts.remove(a1);
        ...
        System.out.println("Writing/Deleting accounts.");
        if (tt)
            data.accounts.add(a2);
        else
            data.accounts.remove(a2);
